"""
Magneti Marelli ECU flasher.

Covers IAW gasoline controllers (4AF, 5AF, 5SF, 7GF) and MJD diesel
controllers (6F3, 8F2, 8F3, 9DF) used across Fiat/Alfa/Lancia/VW LCV.
"""
...
MM_REGIONS: Dict[str, Region] = {
    "IAW4AF":  (0x000000, 0x020000),
    "IAW5AF":  (0x000000, 0x040000),
    "IAW5SF":  (0x000000, 0x040000),
    "IAW7GF":  (0x000000, 0x080000),
    "MJD6F3":  (0x000000, 0x100000),
    "MJD8F2":  (0x000000, 0x100000),
    "MJD8F3":  (0x000000, 0x100000),
    "MJD9DF":  (0x000000, 0x200000),
}

DIESEL_VARIANTS = {"MJD6F3", "MJD8F2", "MJD8F3", "MJD9DF"}


class MagnettiMarelliFlasher(BaseECUFlasher):
    """Flasher for Magneti Marelli IAW and MJD families."""

    def __init__(
        self, connection, variant: str = "MJD8F3", *, logger_=None
    ) -> None:
        super().__init__(connection, logger_=logger_)
        variant = variant.upper().replace(" ", "").replace(".", "")
        # Normalize aliases like "MJD 6F3" or "IAW 5AF".
        variant = variant.replace("IAW", "IAW").replace("MJD", "MJD")
        if variant not in MM_REGIONS:
            raise UnsupportedECUError(f"MM variant {variant} not supported")
        self.ECU_TYPE = variant
        self.variant = variant
        self.BLOCK_SIZE = 0x100 if variant in DIESEL_VARIANTS else 0x80

    def _default_calibration_region(self) -> Region:
        return MM_REGIONS[self.variant]
    ...
    def enter_programming_mode(self) -> bool:
        ...
        if not key_resp or key_resp[0] != 0x67:
            raise SeedKeyFailedError("MM key rejected")
    ...
    def erase_flash_region(self, start_addr: int, length: int) -> bool:
        ...
        if not resp or resp[0] != 0x71:
            raise EraseFailedError(
                f"MM erase NRC at 0x{start_addr:08X}",
                address=start_addr,
            )
        return True
    ...
    def write_flash_block(self, address: int, data: bytes) -> bool:
        ...
        if not resp or resp[0] != 0x74:
            raise WriteFailedError(
                f"MM RequestDownload NRC at 0x{address:08X}",
                address=address,
            )
        counter = 1
        sent = 0
        while sent < len(data):
            ...
            if not td or td[0] != 0x76:
                raise WriteFailedError(
                    f"MM TransferData NRC at 0x{address + sent:08X}",
        ...
        if not end or end[0] != 0x77:
            raise WriteFailedError(
                f"MM TransferExit NRC at 0x{address:08X}",
                address=address,
            )
        return True
